...
class GroupDetailViewController: UIViewController , UITableViewDataSource, UITableViewDelegate{
    ...
    @objc func segmentControlChanged() {
        // Whenever the segment control changes, update the separator style
        updateSeparatorStyle()
        tableView.reloadData() // Reload the table view to apply changes
    }

    func updateSeparatorStyle() {
        // Adjust separator style based on selected segment
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return 0 // Adjust footer height to avoid space between sections
    }
